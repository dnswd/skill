...
- ALWAYS default to smaller, faster models (such as `gemini-3.5-flash` or equivalent) for setup and OAuth checking tasks.
- Keep all explanations and status updates extremely brief: **exactly one sentence** or a very short status line, unless a full code block or error detail is absolutely required.
