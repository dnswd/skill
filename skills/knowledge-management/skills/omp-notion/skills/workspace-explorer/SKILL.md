...
## Performance & Efficiency Constraints (omp)

To minimize LLM call costs and save time in Oh My Pi (omp):
- ALWAYS default to smaller, faster models (such as `gemini-3.5-flash` or equivalent) for workspace exploration and searching tasks.
- Keep all explanations, summaries, and responses extremely brief: **exactly one sentence** summarizing the result, plus any requested direct links, unless a full code block or schema structure is explicitly asked for by the user.
